...
//import { getFoodBanks } from "../database.js"
...
const ViewDonors = () => {
    ...
    const [foodBanks, setFoodBanks] = useState(state.foodbankList) // Stores food listings
    const [retrieveFoodbanks, setRetrieveFoodBanks] = useState(state.getFoodBanks) // Tells when to get food listings
...
//Function refreshMarkers() - Updates foodbankList to get all food banks from the database.
    const refreshMarkers = () => {
    //console.log("Calling refreshMarkers")
    //if (state.getFoodBanks == true){   // If-statement here if calling function infinitely
    console.log("Refreshing food banks...")
    foodbankArray = []
    getFoodBanks().then(function(result){
        // Each element in foodbankArray is contained in an array: [latitude, longitude, fullName, phoneNumber]
        foodbankArray = result
        foodbankArray.forEach(element => console.log(element))

        dispatch({foodbankList: foodbankArray, getFoodBanks: false})
        setRetrieveFoodBanks(false);
        setFoodBanks(foodbankArray)
    }).catch(function(error){
        console.log(error)
    });
    console.log("Refreshed food banks!")
    //}
    }
